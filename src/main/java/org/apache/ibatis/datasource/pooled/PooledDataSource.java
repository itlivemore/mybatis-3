...
public class PooledDataSource implements DataSource {
	...
	public Connection getConnection() throws SQLException {
		// 返回的是包装后的PooledConnection.getProxyConnection(),PooledConnection是一个InvocationHandler
		// 返回代理对象是当连接要关闭时调用Connection.close()方法时不是真的去关闭连接，而是把它放到连接池里去
		...
		while (conn == null) {
			synchronized (state) {
				if (!state.idleConnections.isEmpty()) {
					...
					// 连接池中没有空闲的连接，获取当前正在活动的连接数与设置的最大连接数比较
